...
class AssignTask extends Component {
    ...
    componentDidMount() {
        AuthenticationService.getAdminUsersOnly().then(x => {
            ...
            x.map(
                user => {
                    ret.push({ label: `${user.fullName} (${user.email})`, value: user.email });
                }
            );
            this.setState({ allAdmins: ret });
        });
                    ...
                    <Card>
                        ...
                        <Card.Body>
                            <Formik
                                ...
                                onSubmit={({ employeeName, clientName, startDate, endDate, service, taskName, description }, { setStatus, setSubmitting }) => {                                    
                                    setStatus();            
                                    TaskService.addTask(taskName, description, startDate, endDate, employeeName, clientName, service)
                                        .then(
                                            response => {
                                                if (!response.status) {
                                                    setSubmitting(false);
                                            ...
                                            {this.state.clientName && this.state.clientName != '' &&
                                                <>
                                                    <div className="form-group col-md-12">
                                                        ...
                                                        <Select name="employeeName" onChange={this.onDropDownChange} options={this.state.allAdmins} classNamePrefix="select" className={'form-control' + (errors.employeeName && touched.employeeName ? ' is-invalid' : '')} />
